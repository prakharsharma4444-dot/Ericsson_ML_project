...
function Sidebar({ active, onNavigate, onNewAnalysis, theme }) {
  return (
    <aside className={`w-64 flex flex-col justify-between border-r p-4 transition-colors flex-shrink-0 ${
      theme === 'dark' ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-slate-900 border-slate-800 text-white'
    }`}>
      <div>
        {/* Brand Logo */}
        <div className="flex items-center gap-3 px-3 py-4 mb-4">
          ...
          <span className="font-bold text-lg tracking-wide text-white">ERICSSON</span>
        </div>
