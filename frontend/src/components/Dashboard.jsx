...
function Dashboard({
  ...
  return (
    <div className="space-y-6 relative">
      ...
      {searchQuery && (
        <div className="flex items-center justify-between bg-blue-50/70 border border-blue-200/60 rounded-xl px-4 py-2.5 text-xs text-blue-900">
          <div className="flex items-center gap-2">
            <Filter size={14} className="text-blue-600" />
            <span>
              Filtering cases matching: <strong className="font-semibold text-blue-950">"{searchQuery}"</strong>
            </span>
          </div>
          <button
            onClick={onClearSearch}
            className="flex items-center gap-1 font-semibold text-blue-700 hover:text-blue-900 transition"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-700">Recent Activity (n-1 Date)</h3>
            <span className="text-xs text-slate-400">Click row for raw info</span>
          </div>

          {filteredRecent.length === 0 ? (
            <div className="py-10 text-center space-y-2">
              <SearchX size={28} className="mx-auto text-slate-300" />
              <p className="text-xs font-medium text-slate-500">No matching recent activity found.</p>
            </div>
          ) : (
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-slate-700">Needs Attention (Open Only)</h3>
            </div>
          ...
          {filteredAttention.length === 0 ? (
            <div className="py-10 text-center space-y-2">
              <SearchX size={28} className="mx-auto text-slate-300" />
              <p className="text-xs font-medium text-slate-500">No urgent tickets match your filter.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {filteredAttention.map((c, i) => (
                <div
                  key={c.caseId || i}
                  onClick={() => setSelectedCaseModal(c)}
                  className="flex items-center justify-between text-xs p-2.5 rounded-lg border border-slate-100 hover:border-blue-300 hover:bg-blue-50/50 cursor-pointer transition group"
                >
                  <div>
                    <p className="font-semibold text-blue-600 group-hover:underline">#{c.caseId}</p>
                    <p className="text-red-500 mt-0.5 font-medium">{c.issue}</p>
                  </div>
                  <span className="text-slate-400 text-[11px] group-hover:text-blue-600 font-medium">
                    View Raw Info &rarr;
                  </span>
                </div>
              ))}
      ...
      {selectedCaseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-xl w-full max-h-[90vh] overflow-hidden border border-slate-100 animate-in fade-in zoom-in duration-150">
            <div className="p-5 border-b border-slate-100 flex items-start justify-between bg-slate-50/50">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-bold text-slate-800">Case #{selectedCaseModal.caseId}</h3>
                  {selectedCaseModal.issue && (
                    <span className="text-[11px] font-semibold text-red-600 bg-red-50 border border-red-100 px-2.5 py-0.5 rounded-full">
                      {selectedCaseModal.issue}
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-500 mt-1">{selectedCaseModal.subject || 'No Subject Provided'}</p>
              </div>
              <button
            ...
            <div className="p-5 space-y-4 overflow-y-auto max-h-[calc(90vh-130px)]">
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="font-semibold text-slate-700 text-sm">{selectedCaseModal.status || 'N/A'}</span>
                </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="font-semibold text-slate-700 text-sm">{selectedCaseModal.priority || 'N/A'}</span>
                </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="font-medium text-slate-700">{selectedCaseModal.dateOpen || 'N/A'}</span>
                </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="font-medium text-slate-700">{selectedCaseModal.solutionTarget || 'N/A'}</span>
                </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="font-medium text-slate-700">{selectedCaseModal.contactName || 'N/A'}</span>
                </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                  ...
                  <span className="font-medium text-slate-700">{selectedCaseModal.caseOwner || 'N/A'}</span>
                </div>
              </div>
              ...
              {selectedCaseModal.desc && (
                <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-100 text-xs">
                  ...
                  <p className="text-slate-700 whitespace-pre-wrap leading-relaxed">{selectedCaseModal.desc}</p>
                </div>
              )}
            </div>
            ...
            <div className="p-4 border-t border-slate-100 flex items-center justify-end bg-slate-50/50">
              <button
                onClick={() => setSelectedCaseModal(null)}
                className="px-4 py-2 text-xs font-semibold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 rounded-lg shadow-sm transition"
              >
                Close
              </button>
            </div>
